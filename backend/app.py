...
from backend.data.strategic_groups import get_groups, get_group
...
@app.get("/api/groups")
def list_groups():
    """Return neutral, centrally defined strategic groupings."""
    countries = {item["id"] for item in repository.get_country_index()}
    return jsonify([{**group, "available_members": [member for member in group["members"] if member in countries]} for group in get_groups()])
...
def group_detail(group_id: str):
    ...
    if not group:
        return jsonify({"error": f"Strategic group '{group_id}' not found"}), 404
    countries = {item["id"] for item in repository.get_country_index()}
    return jsonify({**group, "available_members": [member for member in group["members"] if member in countries]})
...
@app.post("/api/search")
def semantic_search():
    """Semantic search over real migrated data (country profiles,
    relationships, history events). Returns actual records with
    record_id, record_type, entity, relevance, source, confidence —
    never a bare generated answer."""
    from backend.analysis import semantic_search as search_module
    body = request.get_json(force=True, silent=True) or {}
    ...
    try:
        results = search_module.search(query, top_k=top_k)
    except Exception as e:
        logger.warning("Semantic search unavailable: %s", e)
        return jsonify({
            "error": "Semantic search model unavailable",
            "detail": "The embedding model could not be loaded (likely no network access to download it on first use). This is a truthful system state, not a fallback to fake results.",
        }), 503
