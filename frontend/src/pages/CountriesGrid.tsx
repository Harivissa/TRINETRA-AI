...
function Card({ profile, index, mode }: { profile: Profile; index: number; mode: ViewMode }) {
  ...
  const landmark = geography && typeof geography === "object" && typeof geography.capital === "string" ? geography.capital : null;
  return <DepthCard><Link to={`/country?id=${profile.id}`} className={`group relative overflow-hidden rounded-md border border-trinetra-border bg-trinetra-panel text-left transition-all hover:border-trinetra-saffron focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-trinetra-saffron ${mode === "list" ? "flex min-h-32" : "aspect-[16/9] min-h-[174px]"}`}>
    <div className={`${mode === "list" ? "w-36 shrink-0" : "absolute inset-0"} bg-cover bg-center opacity-50 transition duration-500 group-hover:scale-105 group-hover:opacity-65`} style={undefined} aria-hidden="true" />
...
export default function CountriesGrid() {
  ...
  const filtered = useMemo(() => countries.filter((profile) => { const haystack = `${profile.name} ${profile.id} ${regionOf(profile.detail)} ${profile.detail?.geography?.capital || ""}`.toLowerCase(); return haystack.includes(query.toLowerCase()) && (region === "All Regions" || regionOf(profile.detail) === region); }), [countries, query, region]);
