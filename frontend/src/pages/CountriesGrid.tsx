...
export default function CountriesGrid() {
  ...
  const load = () => { setLoading(true); setError(false); setApiOnline(false); Promise.all([api.getHealth(), api.getCountries()]).then(async ([health, entries]) => { if (health.status !== "ok") throw new Error("Backend health check failed"); setApiOnline(true); setCountries(await Promise.all(entries.map(async (entry) => { try { return { ...entry, detail: await api.getCountry(entry.id) }; } catch { return entry; } }))); }).catch(() => { setCountries([]); setError(true); setApiOnline(false); }).finally(() => setLoading(false)); };
  useEffect(() => { load(); }, []);
