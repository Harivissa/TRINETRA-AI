...
function Card({ profile, index, mode, onOpen }: { profile: Profile; index: number; mode: ViewMode; onOpen: () => void }) {
  ...
  return (
    <button onClick={onOpen} className={`group relative overflow-hidden border border-trinetra-border bg-trinetra-panel text-left transition-all hover:-translate-y-1 hover:border-trinetra-saffron focus-visible:outline-none ${mode === "list" ? "flex min-h-32 items-stretch" : "min-h-[280px]"}`} aria-label={`Open intelligence profile for ${profile.name}`}>
      ...
      <div className={`relative flex min-w-0 flex-1 flex-col justify-between gap-6 p-5 ${mode === "list" ? "sm:flex-row sm:items-center sm:gap-8" : "min-h-[280px]"}`}>
        <div className="flex min-w-0 flex-col gap-3">
          <div className="flex flex-wrap items-center gap-2 font-mono text-[10px] uppercase tracking-[0.16em] text-trinetra-saffron"><span>{profile.id}</span><span className="text-neutral-600">/</span><span>{region}</span></div>
...
export default function CountriesGrid() {
  ...
  return <div className="min-h-screen bg-trinetra-bg text-neutral-200"><Header /><main className="mx-auto max-w-[1800px] px-5 py-10 sm:px-8 sm:py-16">
