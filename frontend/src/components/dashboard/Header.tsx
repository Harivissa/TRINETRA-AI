...
export default function Header() {
  ...
  const scheduleClose = () => { window.clearTimeout(closeTimer.current); closeTimer.current = window.setTimeout(() => setOpen(false), 320); };

  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      if (event.key === "Escape") { setOpen(false); }
      if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "k") { event.preventDefault(); window.location.assign("/country"); }
    };
    window.addEventListener("keydown", onKey);
    return () => { window.removeEventListener("keydown", onKey); window.clearTimeout(closeTimer.current); };
  }, []);

  const groups = [...NAV, ...SECONDARY];
  return <>
    <div className="sidebar-edge-zone" onMouseEnter={reveal} aria-hidden="true" />
    <aside className={`trinetra-sidebar ${open ? "is-open" : ""}`} onMouseEnter={reveal} onMouseLeave={scheduleClose} aria-label="Intelligence modules">
    ...
    <header className="trinetra-topbar sticky top-0 z-20 border-b border-trinetra-border bg-trinetra-bg/95 backdrop-blur"><div className="mx-auto flex max-w-[1800px] items-center justify-between px-5 py-3 lg:px-8"><Link to="/" className="flex items-center gap-3" aria-label="TRINETRA overview"><span className="flex size-8 items-center justify-center rounded-sm bg-trinetra-saffron text-xs font-bold text-black">TN</span><span><span className="block font-display text-xl leading-none text-white">TRINETRA</span><span className="hidden font-mono text-[8px] uppercase tracking-[.14em] text-neutral-600 sm:block">Strategic intelligence</span></span></Link><nav className="hidden items-center gap-6 md:flex" aria-label="Primary navigation">{NAV.map((item) => <Link key={item.to} to={item.to} className={`border-b-2 py-2 text-sm transition-colors ${active(item.to) ? "border-trinetra-saffron text-white" : "border-transparent text-neutral-400 hover:text-white"}`}>{item.label.replace("Command center", "Overview")}</Link>)}</nav><div className="flex items-center gap-3"><Link to="/country" className="hidden items-center gap-2 text-xs text-neutral-400 hover:text-trinetra-saffron lg:flex"><Search size={16} aria-hidden="true" /> Search intelligence <kbd className="border border-trinetra-border px-1.5 py-0.5 font-mono text-[9px]">Ctrl K</kbd></Link><Link to="/analyze" className="rounded-sm bg-trinetra-saffron px-3 py-2 text-xs font-semibold text-black hover:bg-trinetra-saffronDim">Ask Trinetra</Link><button className="rounded-sm border border-trinetra-border p-2 text-neutral-300 md:hidden" onClick={() => setOpen(!open)} aria-label={open ? "Close navigation" : "Open navigation"} aria-expanded={open}>{open ? <X size={18} /> : <Menu size={18} />}</button></div></div>{open && <nav className="flex flex-col gap-1 border-t border-trinetra-border px-5 py-3 md:hidden" aria-label="Mobile navigation">{groups.map(({ label, to }) => <Link key={label} onClick={() => setOpen(false)} to={to} className={itemClass(to)}>{label}</Link>)}</nav>}</header>
  </>;
}
